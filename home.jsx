...
import { View, Text, StyleSheet, Image, FlatList, TouchableOpacity, ScrollView, RefreshControl, StatusBar, Alert, BackHandler } from 'react-native';
...
const HomeScreen = () => {
  ...
  const checkForNewMovies = async (movies) => {
    if (!movies.length) return;
    const lastMovieTitle = await AsyncStorage.getItem('lastMovieTitle');
    const newestMovie = movies[0]?.title;

    if (newestMovie && newestMovie !== lastMovieTitle) {
      await Notifications.scheduleNotificationAsync({
        content: { title: 'New Movie Released!', body: `${newestMovie} is now available!`, sound: 'default' },
        trigger: null,
      });
      await AsyncStorage.setItem('lastMovieTitle', newestMovie);
    }
  };
